...
| --- | --- | --- | --- |
| D-001 | Create a new repository | Prevent Hearth history and architecture from becoming accidental dependencies | Accepted |
| D-002 | Build a mobile-first PWA | The three household members need phone access; native apps are unnecessary for the first beta | Accepted |
| D-003 | Use Next.js and TypeScript | Align with the stronger Zamkee direction and make domain contracts explicit | Accepted |
| D-004 | Use Supabase Auth and Postgres | Shared multi-phone state needs a real managed backend with household isolation | Accepted |
| D-005 | Keep finances separate | Household participation and divorce-related expense evidence are different trust domains | Accepted |
| D-006 | Separate appreciation from contribution | Cooperation should feel rewarding without making the balance picture dishonest | Accepted |
| D-007 | Require gratitude acknowledgement | Completion should be socially witnessed, not merely checked off | Accepted |
...
| D-022 | Use High Fives as the personal spendable reward and keep Home Energy separate | A completed quest earns High Fives only after another person sends thanks. Home Energy remains the distinct shared household tally; the former unrelated casual-high-five action is retired | Superseded and accepted |
...
| D-026 | Use symmetry for belonging and left alignment for action | Home, shared energy, celebration, and family identity benefit from one calm central axis; quest lists, instructions, and administration remain left-aligned for fast scanning | Accepted |
| D-027 | Separate daily capacity, intentions, and private cycle data | A person may share how much they can carry and which quests they intend to do without exposing sensitive dates; cycle estimates never assign capacity automatically and the moon phase remains a separate visual rhythm | Accepted |
| D-028 | Keep Home status in one compact corner cluster | Capacity, moon phase, and Home Energy are useful ambient signals, but the family shape and primary help action must remain the visual centre of Home. Home Energy is shown as a simple cumulative tally, not as a fraction inside this compact status view | Accepted |
| D-029 | Show opening check-ins only for meaningful events | A generic balance summary creates clutter. The opening message appears for thanks, repairs, or available Treasure and names the action directly | Accepted |
